...
void setup_definitions() {
    ...
    // 1: GRASS (Side = 16, Top = 1, Bottom = 2)
    global.blocks[BlockId::GRASS] = std::make_unique<Block>(BlockId::GRASS, 16);
    global.blocks[BlockId::GRASS]->texture_faces[3] = 1; // Top face (+Y)
    global.blocks[BlockId::GRASS]->texture_faces[2] = 2; // Bottom face (-Y)
...
int main(int argc, char *argv[]) {
    ...
    auto camera = std::make_unique<Camera>(
        glm::vec3(32, 120.5f, 32), glm::radians(90.0f));
    auto hitbox = std::make_unique<Hitbox>(vec3(32, 120, 32), vec3(0.2f, 0.9f, 0.2f));
    ...
    while (!wnd->is_should_close()) {
        ...
        auto delta_seconds = static_cast<float>(raw_delta) / 1'000'000'000.0f;
