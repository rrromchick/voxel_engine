...
void WorldFiles::write_region_to_file(int rx, int rz) {
    auto it = regions.find({ rx, rz });
    if (it == regions.end() || !it->second) {
        return;
    }

    std::span<uint8_t> buffer_span { main_buffer.get(), main_buffer_capacity };
    unsigned int size = write_region(buffer_span, rx, rz, *it->second);
    if (size > 0) {
        File::write_binary_file(get_region_file(rx, rz), buffer_span.first(size));
    }
}
...
void WorldFiles::write() {
    for (auto &[coords, region_ptr] : regions) {
        ...
        write_region_to_file(coords.x, coords.z);
    }
}
